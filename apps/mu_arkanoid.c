...
void arkanoid_window(mu_Context *ctx, int n) {
	...
	/* do window */
	if (mu_begin_window(ctx, "Arkanoid", mu_rect(700, 200, ARKANOID_W+FRAME_SPACE_W, ARKANOID_H+FRAME_SPACE_H))) {
		...
		if (h > ARKANOID_H)
			h = ARKANOID_H;
                mu_draw_image(ctx, arkanoid_buf, mu_rect(rect.x, rect.y, ARKANOID_W, ARKANOID_H), w, h, n);
...
//	n = mu_add_win(arkanoid_window);
	...
	n = mu_add_win("Arkanoid", 700, 200, ARKANOID_W, ARKANOID_H, arkanoid_buf);
